...
const SearchMessages = ({
  ...
  return (
    <div className=" bg-conversation-panel-background flex flex-col border-r border-l border-[#38444d] w-[40rem] md:w-[30rem] sm:max-w-[20rem] xs:!max-w-[17rem] h-screen sticky top-0">
      <div className="h-16 px-4 py-5 flex gap-10 sm:gap-4 items-center bg-panel-header-background text-primary-strong">
        <IoClose
      ...
      <div className="overflow-auto custom-scrollbar h-full">
        <div className="flex items-center flex-col w-full">
          ...
          <span className="mt-10 text-secondary sm:text-xs">
            {!searchTerm.length &&
        ...
        <div className="flex justify-center h-full flex-col">
          ...
          <div className="flex flex-col w-full h-full">
            {searchedMessages.map((message) => (
              <div key={message._id} className="flex cursor-pointer flex-col justify-center hover:bg-panel-header-background w-full px-5 border-b-[0.1px] border-secondary py-5">
                <div className="text-sm text-secondary">
                  {calculateTime(message.createdAt)}
                </div>
                <div className="text-icon-blue">{message.message}</div>
              </div>
            ))}
          </div>
        </div>
